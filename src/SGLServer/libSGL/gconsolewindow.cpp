...
#ifdef SGL_GRAPHICAL_CONSOLE_NO_TOOLBAR
    removeToolbar();
#endif
}
...
void GConsoleWindow::clearConsole() {
    ...
    if (_clearEnabled) {
        // print to standard console (not graphical console)
        printf("%s\n", msg.c_str());
...
void GConsoleWindow::processCommandHistory(int delta) {
    ...
    _commandHistoryIndex = std::min(_commandHistoryIndex, (int) _inputCommandHistory.size());
...
void GConsoleWindow::processUserInputEnterKey() {
    ...
    _cinMutex.lockForWrite();
    _cinQueueMutex.lockForWrite();
    _inputLines.push(_inputBuffer);
    _inputCommandHistory.push_back(_inputBuffer);
...
std::string GConsoleWindow::readLine() {
    ...
    while (!_shutdown && !std::cin.eof()) {
        bool lineRead = false;
        if (!_inputScript.empty()) {
            _cinQueueMutex.lockForWrite();
            line = _inputScript.front();
            _inputScript.pop();
            lineRead = true;
            _cinQueueMutex.unlock();
        ...
        if (!_inputLines.empty()) {
            _cinQueueMutex.lockForWrite();
            if (!_inputLines.empty()) {
                line = _inputLines.front();
                _inputLines.pop();
                lineRead = true;
            }
